...
3. **CMP:** escolher/instalar uma CMP certificada do Google, publicar sua mensagem e carregar seu script independentemente de `ads.js` (evita depender do próprio AdSense para iniciar consentimento). Confirmar o ID IAB na lista atual do Google, configurar `GOLGUESS_CMP_ID`, testar aceitar, recusar, revogar e retorno de visitante em EEE/UK/CH; configurar também Brasil e estados relevantes dos EUA, incluindo opt-out/GPP/GPC quando aplicável. Verificar cookies e rede antes/depois. Só depois definir `GOLGUESS_ADS_ENABLED=1`. O adaptador atual aceita apenas consentimento TCF afirmativo, inclusive fora do EEE; onde a CMP não o fornecer, anúncios ficam bloqueados. Ainda falta o controle real da CMP para reabrir preferências. Analytics permanece desativado até integração e autorização próprias.
