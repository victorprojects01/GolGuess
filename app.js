...
const copy = {
  pt: {
    ...
    recentRoundLabel:num=>`Desafio #${num}`,
  ...
  en: {
    ...
    recentRoundLabel:num=>`Challenge #${num}`,
  ...
  es: {
    ...
    recentRoundLabel:num=>`Desafío #${num}`,
...
const escapeRecent = value => String(value ?? '').replace(/[&<>"']/g, char =>
  ({'&':'&amp;', '<':'&lt;', '>':'&gt;', '"':'&quot;', "'":'&#39;'}[char]));
...
function renderRecentChallenges(items) {
  ...
  container.innerHTML = list.map(item => {
    ...
    const p = Object.fromEntries(Object.entries(item.player || {}).map(([key,value]) => [key,escapeRecent(value)]));
    const tItem = Object.fromEntries(Object.entries(item.team || {}).map(([key,value]) => [key,escapeRecent(value)]));
    const top = item.top10 || {};
    const top10Title = escapeRecent((top.title && (top.title[lang] || top.title.pt || top.title)) || 'Resposta em disputa hoje');
    const top10Leader = top.leader ? `<small>(#1 ${escapeRecent(top.leader)})</small>` : '';

    return `<article class="recent-card">
      <div class="recent-card-top">
        <a class="recent-card-date" href="/arquivo.html?data=${encodeURIComponent(item.dayIso)}">${rel} · ${escapeRecent(item.date)} · ${escapeRecent(roundTxt)}</a>
      ...
      <div class="recent-card-body">
        <div class="recent-entity">
          ...
          <strong class="recent-entity-name">${p.name || 'Resposta em disputa hoje'}</strong>
          <div class="recent-meta-chips" ${item.player ? '' : 'hidden'}>
        ...
        <div class="recent-sub-grid">
          <div class="recent-sub-item">
            ...
            <strong class="recent-sub-value">${tItem.name || 'Resposta em disputa hoje'} <small>${tItem.country ? `(${tItem.country})` : ''}</small></strong>
          </div>
          <div class="recent-sub-item">
